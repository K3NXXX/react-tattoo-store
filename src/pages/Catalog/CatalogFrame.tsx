...
const CatalogFrame: React.FC = () => {
	...
	const catalogGoods = useSelector(
		(state: RootState) => state.categorySlice.catalogGoods
	)
	const filteredCatalogGoods = catalogGoods.filter(
		(good: CategoryItem) => good.category === `${catalog && catalog.filterName}`
	)

	const filteredCatalogGoodsPrice = filteredCatalogGoods.filter(
		(filteredGood: CategoryItem) =>
			parseFloat(filteredGood.price) <= parseFloat(maxPrice) &&
			parseFloat(filteredGood.price) >= parseFloat(minPrice)
	)
	...
	useEffect(() => {
		axios
			.get(
				`https://tattoo-shop-801cb78430a8.herokuapp.com/products/sort?forWho=${filterButton}`
			)
			.then(res => {
	...
	return (
		<div className={style.wrapper}>
			<div className={style.path}>
				<Link to='/react-tattoo-store'>Головна</Link> /<span> Каталог</span> /
				<Link to={`/react-tattoo-store/catalog/${id}`}>
			...
			<div className={style.catalog__list}>
				{filteredCatalogGoodsPrice.length > 0 ? (
					filteredCatalogGoodsPrice.map((good: CategoryItem) => {
